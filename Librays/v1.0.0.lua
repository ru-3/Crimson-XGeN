...
local CRIMSON = Color3.fromRGB(230, 30, 50)
local CRIMSON_DARK = Color3.fromRGB(120, 10, 25)
...
VexUI.Themes = setmetatable({ Crimson = CrimsonTheme }, { __index = function() return CrimsonTheme end })

local function ShineColor()
    return ColorSequence.new({
        ColorSequenceKeypoint.new(0, CRIMSON_DARK),
        ColorSequenceKeypoint.new(0.45, CRIMSON),
        ColorSequenceKeypoint.new(0.5, Color3.fromRGB(255, 255, 255)),
        ColorSequenceKeypoint.new(0.55, CRIMSON),
        ColorSequenceKeypoint.new(1, CRIMSON_DARK),
    })
end

local ShineObjects = {}
game:GetService("RunService").Heartbeat:Connect(function(dt)
    for gradient, speed in pairs(ShineObjects) do
        if gradient.Parent then
            gradient.Rotation = (gradient.Rotation + speed * dt) % 360
        else
            ShineObjects[gradient] = nil
        end
    end
end)

function Utility:Shine(thickness, speed, transparency)
    local gradient = Instance.new("UIGradient")
    gradient.Color = ShineColor()
    gradient.Rotation = 0
    ...
    ShineObjects[gradient] = speed or 90
    return stroke
end

UI.Theme = CrimsonTheme
...
function VexUI:GetThemeProperty(property, theme, fallbackProperty)
    ...
    return resolve(theme, property)
        or resolve(CrimsonTheme, property)
        or (fallbackProperty and (resolve(theme, fallbackProperty) or resolve(CrimsonTheme, fallbackProperty)))
end
...
function Utility:ElText(parent, title, desc, scope)
    ...
    }, { Utility:Padding({ left = 10, right = 6 }) })
...
function Utility:Element(RightScroll, ElementFrame, sizeY, scope)
    local Beeee = VexUI:Create("Frame", {
        ...
        AutomaticSize = "Y",
        Size = UDim2.new(1, -14, 0, sizeY or 40),
        ZIndex = 15,
    })
    ...
    local Inner = VexUI:Create("Frame", {
        ...
        BackgroundTransparency = 1,
        AutomaticSize = "Y",
        Size = UDim2.new(1, -(ElementReservedRight[scope] or 0), 0, math.max((sizeY or 40) - 12, 10)),
        ZIndex = 16,
    }, {
        Utility:ListLayout("V", 1),
        Utility:Padding({ top = 9, right = 6 }),
    })
...
function Text(parent, text, textProps, children)
    ...
    local function Render(newText)
        ...
        for lineIndex, line in ipairs(string.split(newText, "\\n")) do
            ...
            if textProps.TextWrapped then
                local function FixRow()
                    local rowW = row.AbsoluteSize.X
                    if rowW <= 0 then return end
                    local iconsW, labelCount, lastLabel = 0, 0, nil
                    for _, c in ipairs(row:GetChildren()) do
                        if c:IsA("ImageLabel") then
                            iconsW = iconsW + c.AbsoluteSize.X
                        elseif c:IsA("TextLabel") then
                            labelCount += 1
                            lastLabel = c
                        end
                    end
                    if labelCount == 1 and lastLabel then
                        local w = math.max(rowW - iconsW - (iconsW > 0 and 6 or 0), 10)
                        lastLabel.AutomaticSize = Enum.AutomaticSize.Y
                        lastLabel.Size = UDim2.new(0, w, 0, 0)
                    end
                end
                task.defer(FixRow)
                row:GetPropertyChangedSignal("AbsoluteSize"):Connect(FixRow)
            end
        end
    end
    Render(text)
    ...
    do
        ...
        if not ok then
            UIScreen.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
        end
    end

    local ISLAND_SHOWN = UDim2.new(0.5, 0, 0, 12)
    local ISLAND_HIDDEN = UDim2.new(0.5, 0, 0, -60)
...
function UI:CreateWindow(Config)
    local Window = {
        Name = Config.Name or UI.Name,
        Version = Config.Version or UI.Version,
    ...
    if Window.KeySystem and Window.KeySystem.KeyValidator then
        ...
        if not isfolder("CrimsonXGin/" .. (Window.Folder or "Temp")) then makefolder("CrimsonXGin/" .. (Window.Folder or "Temp")) end
        if isfile("CrimsonXGin/" .. (Window.Folder or "Temp") .. "/key.json") then
            KeyFrame.Frame.TextBox.Text = game:GetService("HttpService"):JSONDecode(readfile("CrimsonXGin/" .. (Window.Folder or "Temp") .. "/key.json")).key or ""
        end
        ...
        Kframe1.Frame.Cross.Activated:Connect(function()
            UIScreen:Destroy()
        end)
        ...
        Abutton.Activated:Connect(function()
            ...
            if valid then
                writefile("CrimsonXGin/" .. (Window.Folder or "Temp") .. "/key.json",
        ...
        Gbutton.Activated:Connect(function()
    ...
    local Main = VexUI:Create("Frame", {
        ...
        VexUI:Create("Frame", {
            Size = UDim2.new(0, Window.Size.X.Offset, 0, Window.Size.Y.Offset-8),
        ...
        Utility:Shine(1.4, 60),
    })

    local TopBarF1 = VexUI:Create("Frame", {
        Parent = Main.Frame,
        Size = UDim2.new(0, Window.Size.X.Offset - 182 + 133 + 5, 0, Window.Topbar.Height),
    ...
    TopBarF1.Active = true

    enableDragging(Main, TopBarF1)
    ...
    local LibName = VexUI:Create("TextLabel", {
        ...
        FontFace = Font.new([[rbxassetid://12187365364]], Enum.FontWeight.SemiBold, Enum.FontStyle.Normal),
        Text = Window.Name .. ' <font color="#E61E32" size="10">' .. tostring(Window.Version) .. '</font>',
        AutomaticSize = "Y",
        TextSize = 13,
        ZIndex = 5,
        TextWrapped = true,
    ...
    UserFrame.TextButton.Activated:Connect(function()
    ...
    LeftScroll.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
        LeftScroll.CanvasSize = UDim2.new(0, 0, 0, LeftScroll.UIListLayout.AbsoluteContentSize.Y)
    end)
    ...
    function Window:Tab(Config, type)
        ...
        RightScroll.UIListLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(function()
            RightScroll.CanvasSize = UDim2.new(0, 0, 0, RightScroll.UIListLayout.AbsoluteContentSize.Y)
        end)
       local function SelectTab()
        ...
        function Tab:Paragraph(Config,type)
            ...
            elseif typeof(Paragraph.Color) == "string" then
                ...
                if not ResolvedColor then
                    warn("Crimson X-Gin: Unknown color name '" .. Paragraph.Color .. "'")
                end
            end
            ...
            local ParagraphFrame = VexUI:Create("Frame", {
                ...
                ClipsDescendants = true,
                BackgroundTransparency = 0.5,
                Size = UDim2.new(1, -14, 0, Paragraph.SizeY),
        ...
        function Tab:Button(Config)
            ...
            ButtonTRG.Activated:Connect(function()
        ...
        function Tab:Toggle(Config,type)
            ...
            ToggleTRG.Activated:Connect(function()
        ...
        function Tab:Slider(Config)
            ...
            Inner.Size = UDim2.new(1, -(Slider.Width + 55), 0, math.max(Slider.SizeY - 12, 10))
        ...
        function Tab:Dropdown(Config)
            ...
            DropDownTRG.Activated:Connect(function()
                if Dropdown.Locked then return end
                if DropOpen then
                    closeDropdown()
                else
            ...
            function Dropdown:Refresh(options)
                ...
                for _, Item in ipairs(Dropdown.Option) do
                    ...
                    DropElementTRG.Activated:Connect(function()
                        if Dropdown.Locked then return end
        ...
        function Tab:Space(Value)
            local Space = VexUI:Create("Frame", {
                ...
                ZIndex = 20,
                BackgroundTransparency = 1,
                Size = UDim2.new(1, -14, 0, Value or 2),
            })
        end
        ...
        function Tab:Section(Config)
            ...
            if Section.Icon then
                local SectionIconSize = math.max(16, math.floor(Section.TextSize + 0.5))
                local SectionIconGap = 8
                local DividerWidth = 1
                SectionLabel.UIPadding.PaddingLeft = Section.UIPadding + UDim.new(0, SectionIconSize + SectionIconGap + DividerWidth + SectionIconGap)
                Icon = VexUI:Create("ImageLabel", {
                    AnchorPoint = Vector2.new(0, 0.5),
                    BackgroundTransparency = 1,
                    Position = UDim2.new(0, 0, 0.5, 0),
                    Size = UDim2.new(0, SectionIconSize, 0, SectionIconSize),
                    ZIndex = 20,
                    Parent = SectionLabel,
                    ThemeID = {
                ...
                Divider = VexUI:Create("Frame", {
                    AnchorPoint = Vector2.new(0, 0.5),
                    Position = UDim2.new(0, SectionIconSize + SectionIconGap, 0.5, 0),
                    Size = UDim2.new(0, DividerWidth, 0, SectionIconSize),
                    BorderSizePixel = 0,
                    BackgroundTransparency = 0.45,
                    ZIndex = 20,
                    Parent = SectionLabel,
                    ThemeID = {
                        BackgroundColor3 = "Section.Icon|IconColor"
                    }
                }, {
                    VexUI:Create("UICorner", {
            ...
            SectionTRG.Activated:Connect(function()
                Val = not Val
                Section:Toggle(Val)
            end)
    ...
    function Window:Section(Config)
        ...
        if Section.Icon then
            ...
            Icon = VexUI:Create("ImageLabel", {
                AnchorPoint = Vector2.new(0, 0.5),
                BackgroundTransparency = 1,
                Position = UDim2.new(0, IconLeft, 0.5, 0),
                BorderColor3 = Color3.new(0, 0, 0),
            ...
            Divider = VexUI:Create("Frame", {
                AnchorPoint = Vector2.new(0, 0.5),
                Position = UDim2.new(0, IconLeft + IconSize + IconGap, 0.5, 0),
                Size = UDim2.new(0, DividerWidth, 0, IconSize),
                BorderSizePixel = 0,
                BackgroundTransparency = 0.45,
                ZIndex = 49,
                Parent = SectionBTN,
            ...
            SecTitle.UIPadding.PaddingLeft = UDim.new(0, IconLeft + IconSize + IconGap + DividerWidth + IconGap)
            if IconsV2.Icon(Section.Icon) then
                Icon.Image = IconsV2.GetIcon(Section.Icon)
            elseif string.find(Section.Icon, "rbxassetid://") then
                Icon.Image = Section.Icon
    ...
    function Window:Open()
        Island.Visible = true
        Utility:TweenObject(Island, {Position = ISLAND_HIDDEN}, 0.3)
    ...
    function Window:Close()
        Island.Visible = true
        Utility:TweenObject(Island, {Position = ISLAND_SHOWN}, 0.3)
        ...
        Utility:TweenObject(Island, {Position = ISLAND_SHOWN}, 0.3, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
        task.defer(function()
    ...
    WinElements.ImageButton.Activated:Connect(function()
        if Window.IslandOpen then
            Window:Close()
        end
    end)

    MinzUI.Activated:Connect(function()
    ...
    WinElements.Cross.Activated:Connect(function()
        if CloseConfirmOpen then return end
        CloseConfirmOpen = true
        UI:Dialog({
            Title = "Close Interface",
            Desc = "Are you sure you want to close " .. tostring(Window.Name) .. "?",
            Buttons = {
                {
                    Text = "Close",
                    Callback = function()
                        CloseConfirmOpen = false
                        task.spawn(DestroyWindow)
                    end
                },
                {
                    Text = "Don't Close",
    ...
    function Window:SetTitle(v)
        Window.Name = v
        LibName.Text = v .. ' <font color="#E61E32" size="10">' .. tostring(Window.Version) .. '</font>'
    end
